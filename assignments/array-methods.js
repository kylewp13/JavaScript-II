...
runners.forEach(function(el){
  return fullNames.push(`${el.first_name} ${el.last_name}`);
});
...
firstNamesAllCaps = runners.map(function(el){
  return el.first_name.toLocaleUpperCase();
});
...
runnersLargeSizeShirt = runners.filter(function(el){
...
ticketPriceTotal = runners.reduce(function(acc, a){
 return acc + a.donation;
}, 0);
...
donationTotal = runners.reduce(function(acc, a){
  return acc + a.donation;
}, 0);

averageDonation = donationTotal / runners.length;
...
let companies = [];
runners.forEach(function(el){
  return companies.push(`${el.first_name} Is here representing ${el.company_name}.`);
});
console.log(companies);

// Problem 3
//Create an email list to verify shirt sizes
let runnersEmail = [];

runnersEmail = runners.filter(function(el){
